...
```bash
ng_prepare_benchmark "+config_paths=[benchmarks/aime24_x/config.yaml]"
```

That writes `benchmarks/aime24_x/data/aime24_x_benchmark.jsonl`.
...
```bash
python benchmarks/aime24_x/prepare.py --prompt_language en
```

## Quickstart

```bash
ng_run "+config_paths=[benchmarks/aime24_x/config.yaml,responses_api_models/vllm_model/configs/vllm_model.yaml]"
```
...
```bash
ng_collect_rollouts \
    +agent_name=aime24_x_math_with_judge_simple_agent \
    +input_jsonl_fpath=benchmarks/aime24_x/data/aime24_x_benchmark.jsonl \
    +output_jsonl_fpath=results/aime24_x/rollouts.jsonl \
