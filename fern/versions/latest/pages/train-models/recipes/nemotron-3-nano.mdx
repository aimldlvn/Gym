...
`df -h` should report at least 200 GB available.
...
The clone should leave you on the `nano-v3` branch.
...
The container file should exist (~15 GB), or the registry path should be configured.
...
`uvx --version` should print a version number.
...
When the job completes, you should have `train-split.jsonl` and `val-split.jsonl`.
...
Model files (~59 GB total) should now be present in `model/`.
...
All directories and files should be present at the expected sizes.
...
The script should now exist and be executable.
...
The submission should print a job ID.
...
The job should transition from `PD` to `R`.
...
All actors should report online — 16/16 for 2 nodes, 256/256 for 32 nodes.
...
The rollout percentage should climb steadily and TensorBoard event files should be written.
