...
_ANSWER_FORMAT_REGEXES: dict[str, re.Pattern[str]] = {
    ...
    "fmt_22": re.compile(r"ANSWER IS\s*(.+)", re.S),
    "fmt_23": re.compile(r"Response:\s*(.+)", re.S),
    "fmt_24": re.compile(r"Final Answer\s*->\s*(.+)", re.S),
    "fmt_25": re.compile(r"Final value is:\s*(.+)", re.S),
    "fmt_26": re.compile(r"Correct Answer >>\s*(.+)", re.S),
    "fmt_27": re.compile(r"Answer Value:\s*(.+)", re.S),
    "fmt_28": re.compile(r"Final Answer\s*=\s*(.+)", re.S),
    "fmt_29": re.compile(r"Correct answer is\s*(.+)", re.S),
    "fmt_30": re.compile(r"Final Answer:\s*(.+)", re.S),
}
